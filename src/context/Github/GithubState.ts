...
const GithubState = (props) => { 
    ...
    const getUserRepos = async (username) => {
        setLoading();
        const res = await axios.get(`https://api.github.com/users/${username}/repos?per_page=5&sort=creadted:asc&client_id=${githubClientId}&client_secret=${githubClientSecret}`)
        setLoading(false);
        dispatch({
            type: GET_USER,
            payload: res.data
